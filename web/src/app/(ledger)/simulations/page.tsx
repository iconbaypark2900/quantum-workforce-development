...
import QoblibBenchmarkPanel from "./components/QoblibBenchmarkPanel";

const TABS = [
  { id: "strategy",    label: "Strategy Comparison" },
  { id: "stress",      label: "Stress Scenarios" },
  { id: "walkforward", label: "Walk-Forward Backtest" },
  { id: "qoblib",      label: "QOBLIB Benchmarks" },
] as const;
...
function StressCard({ s, baseVol }: { s: StressScenario; baseVol: number }) {
  ...
  return (
    <div className="bg-ql-surface-container rounded-lg p-5 border border-ql-outline-variant">
      ...
      <p className="text-[10px] text-ql-on-surface-variant mt-1 mb-4">{s.desc}</p>
      <p className={`text-3xl font-headline font-bold ${isGain ? "text-ql-tertiary" : "text-ql-error"}`}>
        {`${impact.toFixed(1)}%`}
      </p>
      ...
      <div className="mt-3 h-1 bg-ql-outline-variant/20 rounded-full overflow-hidden">
        <div
          className={`h-full rounded-full ${isGain ? "bg-ql-tertiary" : "bg-ql-error"}`}
          style={{ width: `${barPct}%` }}
        />
      </div>
    </div>
...
export default function SimulationsPage(props: NextClientPageProps) {
  ...
  return (
    <div className="p-6 lg:p-10 space-y-6">
      <LedgerPageHeader
        title="Simulations"
        subtitle="Compare strategies side-by-side, stress-test under macro scenarios, and run QOBLIB quantum benchmarks"
      />
      <SessionBanner />
      ...
      {/* Tab bar */}
      <div className="flex gap-1 border-b border-ql-outline-variant overflow-x-auto">
        {TABS.map((tab) => (
          <button
            key={tab.id}
            onClick={() => setActiveTab(tab.id)}
            className={[
              "px-4 py-2 text-sm font-bold whitespace-nowrap transition-colors border-b-2 -mb-px",
              activeTab === tab.id
                ? "border-ql-primary text-ql-primary"
                : "border-transparent text-ql-on-surface-variant hover:text-ql-on-surface",
            ].join(" ")}
          >
            {tab.label}
          </button>
          ...
          {scenarios.length > 0 ? (
            <>
              <div className="bg-ql-surface-low rounded-xl p-6 overflow-x-auto">
                <h3 className="font-headline text-lg font-bold mb-1">Strategy Comparison</h3>
                <p className="text-ql-on-surface-variant text-xs mb-4">
                  All objectives run against the same universe and covariance matrix.{" "}
                  <strong>BEST</strong> marks the highest Sharpe in this batch. Shows how each method trades off return, risk, and diversification.
                </p>
                <table className="w-full text-sm font-mono">
                  <thead>
                    <tr>
                      {["Strategy", "Sharpe", "Return", "Volatility", "Positions"].map((h) => (
                        <th
                          key={h}
                          className={`pb-3 text-[10px] uppercase tracking-widest text-ql-on-surface-variant font-bold ${h === "Strategy" ? "text-left" : "text-right"}`}
                        >
                          {h}
                        </th>
                      ))}
                  ...
                  <tbody>
                    {scenarios
                      ...
                      .map((s) => {
                        ...
                        return (
                          <tr key={s.objective} className="hover:bg-ql-surface-container/40 transition-colors">
                            ...
                            <td className={`py-3 text-right ${isBest ? "text-ql-tertiary" : ""}`}>
                              {s.sharpe.toFixed(3)}
                            </td>
              ...
              <div className="bg-ql-surface-low rounded-xl p-6">
                <h3 className="font-headline text-lg font-bold mb-1">Efficient Frontier</h3>
                <p className="text-ql-on-surface-variant text-xs mb-4">
            ...
            <div className="bg-ql-surface-low rounded-xl p-10 text-center text-ql-on-surface-variant text-sm">
      ...
      {activeTab === "stress" && (
        <div className="bg-ql-surface-low rounded-xl p-6 space-y-8">
          <div>
            <h3 className="font-headline text-lg font-bold mb-1">Stress Scenarios</h3>
            <p className="text-ql-on-surface-variant text-xs leading-relaxed">
              Illustrative heuristic based on historical shocks and current volatility — not a live backtest.
              Impact = shock × (0.5 + 3σ_p) × 100, where σ_p is annualized portfolio volatility.
            </p>
          </div>
          <div>
      ...
      {activeTab === "walkforward" && (
        <div className="bg-ql-surface-low rounded-xl p-6 space-y-6">
          <div>
            <h3 className="font-headline text-lg font-bold mb-1">Walk-Forward Backtest</h3>
            <p className="text-ql-on-surface-variant text-xs leading-relaxed">
          ...
          <form onSubmit={(e) => void handleWfSubmit(e)} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <label className="space-y-1 col-span-full">
              <span className="text-[10px] uppercase tracking-widest text-ql-on-surface-variant font-bold">
            ...
            <div className="space-y-6">
              ...
              {wfResult.run_id && (
                <div className="flex items-center gap-3 text-xs">
                  ...
                  <Link href={`/reports/runs/${encodeURIComponent(wfResult.run_id)}`} className="text-ql-primary underline">
                    View full report →
                  </Link>
                </div>
              )}
      ...
      {/* QOBLIB Benchmarks tab */}
      {activeTab === "qoblib" && <QoblibBenchmarkPanel />}
    </div>
  );
}
