...
| **Install** | In the **API** Vercel project settings, set Install Command to: `pip install --upgrade pip && pip install -r deps/requirements-vercel.txt && pip install .` (IBM/Qiskit included). Do **not** put this in root `vercel.json` — it locks Learn/Next projects that cannot override Install. Root `deps/requirements.txt` is for CI/local. |
